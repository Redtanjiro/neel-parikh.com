...
2. **Vanilla HTML/CSS/JS only, with one exception: GSAP.** No React, no other framework, no npm, no bundler, no build step. **GSAP (core + ScrollTrigger + SplitText) is loaded sitewide via the cdnjs CDN, added 2026-07-21** — every page has three `<script src="https://cdnjs.cloudflare.com/ajax/libs/gsap/3.13.0/...">` tags plus a `gsap.registerPlugin(ScrollTrigger, SplitText)` call, right before that page's own inline `<script>` block. **Pinned to 3.13.0, not the 3.12.5 first requested** — `SplitText` was a paid "Club GreenSock" plugin and 404s on cdnjs at 3.12.5; it only became public in 3.13.0. Keep all three files on the same version. **`MorphSVGPlugin` was dropped entirely (2026-07-22)** when the homepage intro moved off the love→heart morph — nothing on the site uses `morphSVG` anymore, so all five pages now load exactly the same three-tag baseline, no per-page exception. No other third-party library — this is a deliberate, single exception, not a general opening. Everything still deploys straight to GitHub Pages with no build step; GSAP is a runtime script tag, not a bundled dependency. Before building a new scroll/reveal/text effect, check whether GSAP already does it rather than hand-rolling another `IntersectionObserver`/`stroke-dashoffset` version — but the existing hand-rolled systems (hero pose scrub, device-mockup parallax, footer pill drop, line-draw hovers) stay as they are; this isn't a mandate to port them. **"My Process" is a pinned, vertical, scroll-scrubbed ScrollTrigger section (desktop) that stacks unpinned on mobile, driven by GSAP core + ScrollTrigger via the existing sitewide CDN tags** — `prefers-reduced-motion` falls back to a fully static render (full path drawn, all nodes amber, all cards at full opacity). **The homepage intro (`#hero`'s `.intro-glass`) is a second pinned/scrubbed ScrollTrigger, ahead of "My Process" in scroll order** — see the preloader/intro entry below for what it does; its pin consumes real scroll distance before hero starts scrolling normally, which is why the hero pose-scrub (`#pose-scrub`) measures `#about`'s actual position at runtime instead of assuming hero sits at document top 0. If you add a third pinned section anywhere on `/`, re-verify all three ranges in sequence — this has already broken once (see `BUILD-SPEC-INTRO.md`'s Build notes).
...
`Fraunces` and `Caveat` exist but are **About-page only**, with one exception: `index.html`'s frosted-glass intro uses `Fraunces` italic (`var(--font-voice)`) for the whole "hand drawn with love" signature over the hero — nowhere else on the homepage. Don't import either family anywhere beyond those two spots.
...
| `BUILD-SPEC-INTRO.md` | Homepage intro v3 — no more black screen or preloader-style blocking. A frosted-glass panel (`backdrop-filter` blur + paper tint) pins over the real hero and clears via scroll: "hand drawn with love" writes in char-by-char, a standalone amber heart (`Vector.svg`) spins in beside it, both fade, then the glass opacity-fades to reveal hero. `MorphSVGPlugin` dropped entirely — nothing on the site uses `morphSVG` anymore. | Built |
