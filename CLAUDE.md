...
2. **EMF-ACE confirms** — India→Abu Dhabi remote framing is now confirmed (designed remotely from India, environmental branding executed on-site in a single day) and no longer marked. **Studio name confirmed as "Media Mushroom" (2026-07-25)** — the hero meta Role line's `[CONFIRM]` gold badge/span is gone, now plain text; the now-unused `.ea-confirm` CSS rule was removed too. One placeholder remains (`emf-award-or-spread`, optional) — everything else uses real campaign assets already in `work/emf-ace/assets/`.
...
**Specs marked ⛔ below have been moved to `_archive/specs/`** (cleaned up 2026-07-25) — they're kept for history (later specs' "supersedes X" notes still make sense), not because anything in them is still buildable. Everything else lives in the repo root as before.
...
| `_archive/specs/BUILD-SPEC-FUTEE-MOVES.md` | Futee rebuilt as a four-move argument, locked copy | ⛔ Superseded by `BUILD-SPEC-FUTEE-V2.md` (new approved copy + asset treatment) |
...
| `_archive/specs/BUILD-SPEC-HERO-SCROLL-TRANSITION.md` | Illustration sticks and scrolls with the visitor from hero into About via a two-column landing panel (desktop only) — supersedes §4/§6 of `BUILD-SPEC-ABOUT.md` | ⛔ Superseded by `BUILD-SPEC-HERO-ABOUT-COLLAPSE.md` |
| `_archive/specs/BUILD-SPEC-HERO-ABOUT-COLLAPSE.md` | Hero pose-scrub retargeted to dissolve into the nav pill instead of landing beside About; word-staggered bio reveal | ⛔ Never pushed — corrected by Neel after seeing screenshots, superseded by `BUILD-SPEC-HERO-SPLIT-REVEAL.md` before shipping |
...
| `BUILD-SPEC-EMF-ACE.md` | EMF ACE case study, old 7-section → four-move; real campaign assets used where they exist | Built — both facts once flagged `[CONFIRM]` (India→Abu Dhabi remote framing, studio name) are now confirmed |
| `BUILD-SPEC-CSEDS-MOVES.md` | CSEDS case study, old 8-section → four-move; all copy is `[COPY TBD]`, real assets reused | Built |
| `_archive/specs/BUILD-SPEC-PRELOADER.md` | Homepage-only preloader — black hold → hand-drawn mug line-draw → signature → dissolve into the hero | ⛔ Superseded by `BUILD-SPEC-PRELOADER-V2.md` |
| `_archive/specs/BUILD-SPEC-PRELOADER-V2.md` | Homepage preloader v2 — GSAP + MorphSVGPlugin, "love" wordmark morphs its "o" into a heart, then signature writes in letter by letter | ⛔ Superseded by `BUILD-SPEC-INTRO.md` — the black-screen blocking preloader is gone entirely |
| `_archive/specs/BUILD-SPEC-INTRO.md` | Homepage intro v3 — a frosted-glass panel that pinned over the real hero and cleared via scroll, "hand drawn with love" signature + spinning heart | ⛔ Removed entirely (2026-07-22), by request — the hero now renders with no intro/preloader of any kind. `MorphSVGPlugin` stays dropped (nothing on the site used it even while this spec was live). |
...
| `BUILD-SPEC-CASE-STUDY-ELEVATION.md` | Stat strips + next-project chaining for the four case-study pages — each page already has a `.{prefix}-meta` hero definition-list, so this designs a second "monument stats" strip that needs visual reconciliation, not a blind bolt-on | ⬜ Not built — was missing from this table entirely until the 2026-07-25 cleanup pass; see HANDOFF's open-work list |
...
/contact.html      Contact         (not built — nav "Contact" is a mailto: link instead, see below)
...
**Nav:** Work · About · Contact · Resources. Work anchor-scrolls to Chosen Work (does *not* go to `/work.html`). **About anchor-scrolls to `#about` on the homepage** (not a separate route — see `BUILD-SPEC-ABOUT.md`). **Contact → `mailto:neelparikh7@gmail.com` (2026-07-25), not `/contact.html`** — `/contact.html` is still unbuilt, so the nav link opens the visitor's mail client directly instead of a dead route; same href on all six live pages (`/`, `/resources.html`, all four `/work/*.html`). Resources → `/resources.html`. **Nothing in the nav points to `/work.html`** — it's reached only via "See all work".
