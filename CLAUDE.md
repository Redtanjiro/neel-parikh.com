...
`Fraunces` and `Caveat` exist but are **About-page only** — the one prior exception (the homepage frosted-glass intro's `Fraunces` signature) no longer applies now that the intro is removed; the Google Fonts import in `index.html` no longer requests `Fraunces` at all. Don't import either family anywhere beyond the About page.
